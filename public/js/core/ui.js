    ...
    if (field.type === 'boolean') {
        ...
        input = `
            <div class="d-flex align-items-center">
                <input type="range" class="form-range flex-grow-1" id="f_${id}" 
                       min="${field.min || 0}" max="${field.max || 5}" 
                       value="${currentVal}" 
                       oninput="document.getElementById('d_${id}').innerText=this.value">
                <span class="fw-bold ms-2" id="d_${id}" 
                      style="min-width:1.5em; text-align: right; ${isPenalty ? 'color: red !important;' : 'color: var(--bs-primary);'}">
                    ${currentVal}
                </span>
            </div>`;
        ...
        labelContent = `
            <div class="d-flex justify-content-between align-items-center w-100">
                ...
                <button type="button" class="btn btn-success btn-sm mb-0 p-0 fw-bold d-flex align-items-center justify-content-center" 
        ...
        input = `<input type="number" class="form-control form-control-sm" id="f_${id}" 
