...
async function run() {
    ...
    // 1. Select Game
    console.log("Selecting Game p16 (Tangled Treasure)...");
    // Using a more robust selector since names might have special chars
    await page.click('button:has-text("Tangled Treasure")');
    await sleep(waitTime);
    ...
    // 3. Fill Scores
    console.log("Filling scores for p16...");
    
    // Patrol Sprirt
    await page.fill('#f_patrol_sprirt', '5');
    await sleep(waitTime);
    // Puzzle Points Completed
    await page.fill('#f_puzzle_points_completed', '5');
    await sleep(waitTime);
