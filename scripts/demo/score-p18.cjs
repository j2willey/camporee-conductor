...
async function run() {
    ...
    // 1. Select Game
    console.log("Selecting Game p18 (Game 18. Blind Mindfield)...");
    // Using a more robust selector since names might have special chars
    await page.click('button:has-text("Game 18. Blind Mindfield")');
    await sleep(waitTime);
    ...
    // 3. Fill Scores
    console.log("Filling scores for p18...");
    
    // Patrol Sprirt
    await page.fill('#f_patrol_sprirt', '5');
    await sleep(waitTime);
    // Scouts Moving
    await page.fill('#f_scouts_moving', '5');
    await sleep(waitTime);
    // Hits
    await page.fill('#f_hits', '5');
    await sleep(waitTime);
