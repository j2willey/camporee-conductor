...
async function run() {
    ...
    // 1. Select Game
    console.log("Selecting Game p20 (Walking A Frame)...");
    // Using a more robust selector since names might have special chars
    await page.click('button:has-text("Walking A Frame")');
    await sleep(waitTime);
    ...
    // 3. Fill Scores
    console.log("Filling scores for p20...");
    
    // Patrol Sprirt
    await page.fill('#f_patrol_sprirt', '5');
    await sleep(waitTime);
    // Time in Seconds
    await page.fill('#f_time_in_seconds_mm', '01');
    await page.fill('#f_time_in_seconds_ss', '23');
    await sleep(waitTime);
