...
async function run() {
    ...
    // 1. Select Game
    console.log("Selecting Game p4 (Coral Reef Construction)...");
    // Using a more robust selector since names might have special chars
    await page.click('button:has-text("Coral Reef Construction")');
    await sleep(waitTime);
    ...
    // 3. Fill Scores
    console.log("Filling scores for p4...");
    
    // Correct Assembly
    await page.fill('#f_correct_assembly', '5');
    await sleep(waitTime);
    // "repacking Tent"
    await page.fill('#f_repacking_tent', '5');
    await sleep(waitTime);
    // Time
